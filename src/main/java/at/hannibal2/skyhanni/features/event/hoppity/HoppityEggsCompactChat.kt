...
object HoppityEggsCompactChat {
    ...
    private fun createCompactMessage(): String {
        ...
        return if (hoppityDataSet.duplicate) {
            val dupeChocFormat = hoppityDataSet.lastDuplicateAmount?.shortFormat() ?: "?"
            ...
            val rarityFormat = if (showDupeRarity) "${hoppityDataSet.lastRarity} " else ""
            ...
            "$mealNameFormat! §7Duplicate $rarityFormat${hoppityDataSet.lastName}$dupeNumberFormat " +
